...
<html lang="en">
	...
	<body>
        ...
        <!-- PROGRESS BAR -->
		<div class="container container-fluid mb-3" id="container-progress">
            <!-- CIRCLE -->
            ...
            <span class="title second " id="active-title" style="cursor: default;">Submit Grades</span>         
		...
		<!-- FEEDBACK -->
        <center>
            <div class="contaier container-fluid mt-5" >
		<?php
            $check_status = mysqli_query($connection,"SELECT * FROM tblstudent_grade_sub WHERE student_id_fk='$studentid'");
            while($fa = mysqli_fetch_array($check_status))
            {
                $get_status = $fa['submission_status'];
            }
            if($get_status == "Pending")
			{
		?>
                <p class="fw-bold bg-light w-75 mt-3" id="feedback" style="cursor: default;">Please wait the approval of your Grades by your Respective Adviser.</p>   
        <?php
			}     
            if($get_status == "Disapproved")
			{
		?>
                <p class="fw-bold bg-light w-75 mt-3" id="feedback" style="cursor: default;">Your Grades has been Dispproved. Please resubmit a valid or Updated Grades.<br> Click the <span><a href="student-grade-input.php">Grades</a> </span>  button below.</p>
        <?php
			}
        ?>
            </div>
